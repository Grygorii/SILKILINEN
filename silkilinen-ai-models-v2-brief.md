# Brief: Update AI Model prompts to v2 + add product/lifestyle templates

## Context

The 5 AI Model identity prompts in the SILKILINEN database (Aoife,
Charlotte, Sofia, Maya, Yuki) have been rewritten as v2 to fix
quality issues identified during a real-world test today. The v1 prompts
produced photographs where the model dominated the visual hierarchy
instead of the garment, plus exhibited identity drift, prop drift
("pyjamas appearing in lifestyle shots that should show robes"), and
tone misalignment with the brand voice.

This brief implements three changes:

1. Replace the existing `prompt` field for each of 5 models with the v2
   versions (full text in this brief).
2. Add two new fields to the `AiModel` schema:
   - `productShotPromptTemplate` — the template used for actual product
     photography after identity is locked
   - `lifestyleShotPromptTemplate` — the engineered painter's-studio
     lifestyle template that avoids known failure modes
3. Unlock all 5 models (set `locked: false`) so the admin can regenerate
   reference images using the new prompts.

The new fields can be the same value across all 5 models (they are
generic templates, not model-specific). Future briefs may make them
per-model if needed.
...
In `backend/models/AiModel.js`, add two new fields:

```js
productShotPromptTemplate: {
  type: String,
  default: null,
  // Template used for product photography. References model identity
  // separately and emphasises garment fidelity.
},

lifestyleShotPromptTemplate: {
  type: String,
  default: null,
  // Template for lifestyle shots. Engineered to avoid bedroom/sleepwear
  // context drift by setting scenes in a Donegal painter's studio.
}
```

If schema migration tooling is in use, run a migration. If not, the
new fields will simply be `null` for existing documents until populated
by this brief's update script.

### Section 2 — Update script

Create `backend/scripts/updateAiModelPromptsV2.js` that:

1. Connects to MongoDB using `MONGODB_URI` env var
2. Updates each of the 5 models with:
   - The v2 `prompt` field (full text below)
   - The shared `productShotPromptTemplate` (full text below)
   - The shared `lifestyleShotPromptTemplate` (full text below)
   - Sets `locked: false` so admin can regenerate the reference image
3. Logs each successful update
4. Logs warnings if any of the 5 models is not found
5. Disconnects cleanly on completion
...
const MODEL_PROMPTS_V2 = {
  'Aoife': `<paste full Aoife v2 prompt from Section 3 below>`,
  'Charlotte': `<paste full Charlotte v2 prompt from Section 3 below>`,
  'Sofia': `<paste full Sofia v2 prompt from Section 3 below>`,
  'Maya': `<paste full Maya v2 prompt from Section 3 below>`,
  'Yuki': `<paste full Yuki v2 prompt from Section 3 below>`
};

const PRODUCT_SHOT_TEMPLATE = `<paste full template from Section 4>`;
const LIFESTYLE_SHOT_TEMPLATE = `<paste full template from Section 5>`;
...
### Section 3 — The v2 model prompts (full text)

Each of these replaces the existing `prompt` field for the named model.
Pasted as exact strings — no edits, no condensation.

#### Aoife v2

```
A 28-year-old fashion model with quiet Irish beauty. The brand's signature face — chosen not because she dominates the frame, but because she lets the garment breathe.
...
```
A 30-year-old British fashion model with refined sophistication. The brand's face for considered intimate apparel — chosen for her stillness, not her drama.
...
```
A 28-year-old Italian fashion model with continental warmth. The brand's face for relaxed luxury — chosen for her grounded ease, not for sensual performance.
...
```
A 27-year-old mixed-heritage American fashion model with contemporary calm. The brand's face for the US and modern global markets — chosen for her grounded presence, not for cool detachment.
...
```
A 28-year-old Japanese fashion model with serene minimalist presence. The brand's face for accessories and considered pieces — chosen for her stillness, which is her entire art.
...
### Section 4 — The product shot template (shared across all models)

Stored in the new `productShotPromptTemplate` field. Same value for all 5 models.

```
PRODUCT SHOT — [SHOT TYPE]
...
### Section 5 — The lifestyle shot template (shared across all models)

Stored in the new `lifestyleShotPromptTemplate` field. Same value for all 5 models.

```
LIFESTYLE SHOT — DONEGAL PAINTER'S STUDIO
...
### Section 6 — How to run the script

```bash
cd backend
MONGODB_URI=<railway_mongo_uri> node scripts/updateAiModelPromptsV2.js
```

Or if dotenv loads MONGODB_URI from `.env` automatically:

```bash
cd backend
node scripts/updateAiModelPromptsV2.js
```

## Verification

### Schema verification
- [ ] `backend/models/AiModel.js` includes `productShotPromptTemplate`
      field (String type, default null)
- [ ] `backend/models/AiModel.js` includes `lifestyleShotPromptTemplate`
      field (String type, default null)
- [ ] No other schema fields are altered

### Script verification
- [ ] `backend/scripts/updateAiModelPromptsV2.js` exists
- [ ] All 5 model prompts (Aoife, Charlotte, Sofia, Maya, Yuki) are
      embedded as exact strings — no condensation, no edits
- [ ] Both shared templates (product shot, lifestyle shot) are embedded
      as exact strings
- [ ] Script logs success/failure for each model
- [ ] Script disconnects from MongoDB cleanly

### Database verification (after running)
- [ ] All 5 model documents have updated `prompt` field with v2 text
- [ ] All 5 model documents have populated `productShotPromptTemplate`
- [ ] All 5 model documents have populated `lifestyleShotPromptTemplate`
- [ ] All 5 model documents have `locked: false`
- [ ] No other model fields (referenceImageUrl, etc.) were altered

### Admin UI verification
- [ ] Visit `/admin/models` — all 5 models appear with `locked: false`
      indicator
- [ ] Each model's prompt field, when expanded, shows the new v2 text
- [ ] Existing reference images are still present (we did not clear
      them — admin will regenerate manually)

## Out of scope

- Do NOT change the existing `referenceImageUrl` field — admin will
  regenerate references manually through the UI
- Do NOT change any other AiModel collection fields beyond the four
  specified (prompt, productShotPromptTemplate, lifestyleShotPromptTemplate,
  locked)
- Do NOT modify the admin UI for AI Models in this brief — the new
  template fields will be wired into the UI in a future brief
- Do NOT regenerate any reference images automatically
- Do NOT touch any other model in the codebase (Product, Order,
  Customer, Visit, etc.)
- Do NOT add tests in this brief unless they're trivial schema tests
...
3. Output of running the script — specifically, the success/failure
   log for each of 5 models
4. Confirmation that all verification steps pass
5. Any deviations or surprises (if any)
6. `git diff --stat` summary

After the script runs successfully, the admin user (Гриша) will:
1. Navigate to `/admin/models`
2. For each of the 5 models, click "Generate Reference Photo" using
   Premium 4K quality (~€0.24 each, ~€1.20 total)
3. Review each generated reference for v2 quality criteria
4. Re-lock each model once approved

This admin step is OUT OF SCOPE for VS Code Claude — only the database
update is in scope.
