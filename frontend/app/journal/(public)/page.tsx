  ...
  return (
    <main style={{ maxWidth: 900, margin: '0 auto', padding: '80px 24px 120px' }}>
      <div style={{ textAlign: 'center', marginBottom: 64 }}>
        ...
        <p style={{ fontSize: 14, color: '#6b5f52', letterSpacing: '1px' }}>
          Stories from Donegal
        </p>
      </div>

      {articles.length === 0 ? (
        <p style={{ textAlign: 'center', color: '#9a8e82', fontSize: 15, fontStyle: 'italic' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 32 }}>
          {articles.map(a => (
            <Link key={a._id} href={`/journal/${a.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <article>
                <div style={{ height: 200, background: '#f0ede8', overflow: 'hidden', marginBottom: 20 }}>
                  {a.heroImage?.url && (
                ...
                <p style={{ fontSize: 11, color: '#9a8e82', letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: 8 }}>
                  {fmtDate(a.publishedAt)}
                  {a.readingTimeMinutes && ` · ${a.readingTimeMinutes} min read`}
                </p>
                <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 22, fontWeight: 400, margin: '0 0 10px', lineHeight: 1.3, color: '#1a1510' }}>
                  {a.title}
                </h2>
                <p style={{ fontSize: 14, color: '#6b5f52', lineHeight: 1.6, margin: '0 0 14px' }}>
                  {a.excerpt}
                </p>
