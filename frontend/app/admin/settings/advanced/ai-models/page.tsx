...
export default function ModelsPage() {
  ...
  function openEdit(m: AiModel) { setEditing({ ...m }); setIsNew(false); }
        ...
        {models.map(model => (
          <div key={model._id} className={`${styles.card} ${!model.active ? styles.cardInactive : ''}`}>
            ...
            <div className={styles.cardBody}>
              ...
              {model.useCases.length > 0 && <p className={styles.useCases}>{model.useCases.join(' · ')}</p>}
            </div>
            <div className={styles.cardActions}>
              ...
              <button
                ...
                disabled={generatingId === model._id}
              >
                {generatingId === model._id ? 'Generating…' : `✨ Generate (€${TIER_COSTS[refTier].toFixed(2)})`}
              </button>
              <div className={styles.actionRow}>
                <button className={styles.editBtn} onClick={() => openEdit(model)}>Edit</button>
                <button className={styles.lockBtn} onClick={() => handleToggleLock(model)} title={model.locked ? 'Unlock model' : 'Lock model'}>
                  {model.locked ? '🔓' : '🔒'}
                </button>
                <button className={styles.activeBtn} onClick={() => handleToggleActive(model)} title={model.active ? 'Deactivate' : 'Activate'}>
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.field}>
                ...
                <textarea
                  ...
                  placeholder="The locked text description used in every generation."
                />
                  ...
                  <div className={styles.checkGroup}>
                    {USE_CASE_OPTIONS.map(uc => (
                      <label key={uc} className={styles.checkLabel}>
                        <input type="checkbox" checked={(editing.useCases || []).includes(uc)} onChange={() => toggleArrayField('useCases', uc)} />
                        {uc}
                      </label>
                    ))}
                  </div>
