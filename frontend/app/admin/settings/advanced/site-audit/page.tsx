...
import styles from '../../../site-audit/page.module.css';
...
type Audit = {
  ...
  synthesis?: {
    headline?: string;
    priorities?: { issue: string; severity?: string; likelyCause?: string; whereToLook?: string; fix?: string }[];
    noise?: string;
  } | null;
  triggeredBy?: string;
};
  ...
  return (
    <div className={`${styles.findingRow} ${finding.status !== 'open' ? styles.findingResolved : ''}`}>
      ...
      <div className={styles.findingActions}>
        {finding.status === 'open' ? (
          <>
            <button className={styles.btnFixed} onClick={() => setStatus('fixed')} disabled={updating}>Fixed</button>
            <button className={styles.btnWontFix} onClick={() => setStatus('wont_fix')} disabled={updating}>Won&apos;t fix</button>
          </>
        ) : (
...
export default function SiteAuditPage() {
  ...
  useEffect(() => { if (selectedId) loadAudit(selectedId); }, [selectedId, loadAudit]);
  ...
  async function triggerAudit() {
    setRunning(true);
    try {
      const res = await fetch(`${API}/api/admin/site-audit`, { method: 'POST', credentials: 'include' });
      if (res.ok) {
      ...
      <div className={styles.page}>
        ...
        <div className={styles.body}>
          ...
          <main className={styles.main}>
            {!selectedId && (
              <div className={styles.emptyState}><p>Run an audit to inspect your site</p></div>
            )}
            {audit && (
              <>
                  ...
                  <>
                    {audit.synthesis && (audit.synthesis.headline || audit.synthesis.priorities?.length) && (
                      <div style={{ border: '1px solid var(--color-line)', background: 'var(--color-bg)', padding: '16px 18px', marginBottom: 16 }}>
                        <p style={{ fontSize: 11, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'var(--admin-ink-muted)', margin: '0 0 8px' }}>✦ AI reading — likely causes &amp; where to look</p>
                        ...
                        {audit.synthesis.priorities?.map((p, i) => (
                          <div key={i} style={{ borderLeft: `3px solid ${p.severity === 'critical' ? 'var(--color-danger)' : p.severity === 'warning' ? 'var(--admin-warning)' : 'var(--admin-ink-muted)'}`, padding: '6px 0 6px 12px', margin: '8px 0' }}>
                            ...
                            {p.likelyCause && <p style={{ fontSize: 12.5, color: 'var(--admin-ink-muted)', margin: '3px 0 0' }}>Likely cause: {p.likelyCause}</p>}
                            {p.whereToLook && <p style={{ fontSize: 12.5, color: 'var(--admin-ink-muted)', margin: '2px 0 0' }}>Where to look: <code>{p.whereToLook}</code></p>}
                            {p.fix && <p style={{ fontSize: 12.5, color: 'var(--color-success)', margin: '2px 0 0' }}>→ {p.fix}</p>}
                          </div>
                        ))}
                        {audit.synthesis.noise && <p style={{ fontSize: 12, color: 'var(--admin-ink-muted)', margin: '10px 0 0', fontStyle: 'italic' }}>Likely noise: {audit.synthesis.noise}</p>}
                      </div>
                    )}
                    {openCritical > 0 && (
                    ...
                    <div className={styles.findings}>
                      {filteredFindings.length === 0 && <p className={styles.empty}>No findings match this filter</p>}
                      {filteredFindings.map((finding, i) => {
                        const realIdx = audit.findings.indexOf(finding);
                        return (
                          <FindingRow key={i} finding={finding} idx={realIdx} auditId={audit._id} onStatusChange={handleStatusChange} />
                        );
                      })}
                    </div>
