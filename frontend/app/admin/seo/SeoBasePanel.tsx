...
// THE SEO BASE (a tab inside the SEO panel) — the founder's "site plan". One
// table of every indexable URL with its meta title + description, wherever that
// meta lives, colour-coded by snippet health, editable inline. Includes the
// safe auto-fix (fill only MISSING meta) and a per-row Yoast-style Preview &
// checks. Saving routes each change back to the right store.
...
export default function SeoBasePanel() {
  ...
  async function save(r: Row) {
    ...
    try {
      ...
      const neutral = r.type === 'page' && !data.metaTitle && !data.metaDescription;
      setRows(prev => prev.map(x => x === r ? {
        ...x,
        title: data.metaTitle, titleLen: (data.metaTitle || '').trim().length, titleHealth: titleHealth(data.metaTitle, neutral),
        description: data.metaDescription, descLen: (data.metaDescription || '').trim().length, descHealth: descHealth(data.metaDescription, neutral),
      } : x));
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: muted, fontStyle: 'italic', maxWidth: 700, margin: '0 0 4px' }}>
        Your site plan — every indexable page in one place. Green is healthy, amber is a nudge, red needs a title or
        description, grey means a page is using its built-in default. Edit any line; it saves back to wherever that
        page&apos;s SEO lives.
      </p>

      <div style={{ display: 'flex', gap: 20, marginTop: 12, fontSize: 13, color: muted, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 10, marginTop: 14 }}>
          {shown.map(r => {
            ...
            const tHealth = titleHealth(c.title, isPage && !c.title && !c.description);
            const dHealth = descHealth(c.description, isPage && !c.title && !c.description);
            ...
            return (
              <div key={k} style={{ border, background: '#fff', padding: '14px 16px' }}>
                ...
                <Field label="Meta description" value={c.description} max={DESC_MAX} healthColor={health[dHealth]} textarea onChange={v => setField(r, 'description', v)} />
                ...
                {openKey === k && (
                  <SeoChecks title={c.title} description={c.description} url={r.url}
...
// Mirror of the backend thresholds. `neutral` = a code-defaulted page: an empty
// field there is grey, not red.
function titleHealth(t: string, neutral?: boolean): Health {
  const n = (t || '').trim().length;
  if (n === 0) return neutral ? 'muted' : 'bad';
  if (n > 60 || n < 25) return 'warn';
  return 'good';
}
function descHealth(d: string, neutral?: boolean): Health {
