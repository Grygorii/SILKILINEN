            ...
            <section className={styles.section}>
              ...
              {loadingProducts ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {products.map(p => (
                    <div key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'space-between', borderBottom: '1px solid var(--admin-line)', paddingBottom: 8 }}>
                      <a href={`/admin/products/${p._id}`} style={{ flex: 1, color: 'var(--admin-ink)', textDecoration: 'none', fontSize: 14 }}>{p.name}</a>
                      <select
                        className={styles.select}
