/**
 * Idempotent seed: creates the three placeholder blog articles from blogPosts.ts
 * as proper JournalArticle documents, status 'draft'. Safe to re-run.
 * Run: node backend/scripts/seedJournalArticles.js
 */
...
async function seed() {
  ...
  for (const def of ARTICLES) {
    const existing = await JournalArticle.findOne({ slug: def.slug });
    if (existing) {
      console.log(`${def.slug}: exists — skipping`);
      continue;
    }
    await JournalArticle.create({ ...def, status: 'draft', author: 'Sabreen' });
    console.log(`${def.slug}: created ✓`);
  }
